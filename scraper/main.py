...
def prepare_products():
    ...
    for cinema in cinemas: 
        ...
        for seance in seances:
            ...
            products[movie_for_cinema].add_seance(seance.get_time().replace(':', '\'') + ' ' + seance.get_date())
    ...
    with open("combinations.csv", mode='w', newline='', encoding='utf-8') as elements_file:
        ...
        elements_writer.writerow(['Product ID*',attribute, 'Value (Value:Position)*'])

        for product in products.values():
            i = 0
            for seance in product.get_seance():
                elements_writer.writerow([product.get_id(), attribute_name, seance + f':{i}'])
                i = i + 1
